...
def yuklash():
    try:
        with open("ma'lumotlar.txt", "r", encoding="utf-8") as f:
            return [qator.strip() for qator in f if qator.strip()]
    except FileNotFoundError:
        return []

def saqlash():
    with open("ma'lumotlar.txt", "w", encoding="utf-8") as f:
        for v in vazifalar:
            f.write(v + "\n")
...
def main():
    ...
    while True:
        ...
        if tanlov == "0":
            saqlash()
            print("📁 Vazifalar saqlandi. Dasturdan chiqildi.")
            break
        elif tanlov == "1":
            vazifa_qoshish()
        elif tanlov == "2":
